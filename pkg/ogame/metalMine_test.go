...
func TestMetalMineConstructionTime(t *testing.T) {
	mm := newMetalMine()
	assert.Equal(t, 8550*time.Second, mm.ConstructionTime(20, 7, Facilities{RoboticsFactory: 3}, false, NoClass))
	assert.Equal(t, 30*time.Second, mm.ConstructionTime(4, 6, Facilities{}, false, NoClass))
}
